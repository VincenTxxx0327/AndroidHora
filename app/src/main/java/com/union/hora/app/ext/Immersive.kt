 ...
 * 不完整，待修复
 */
fun Activity.immersiveAbove15NavigationBar(callback: (() -> Unit)? = null) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.VANILLA_ICE_CREAM) {
        val view = (window.decorView as ViewGroup).getChildAt(0)
        ViewCompat.setOnApplyWindowInsetsListener(view) { v, insets ->
            val statusBars = insets.getInsets(WindowInsetsCompat.Type.statusBars())
            val navigationBars = insets.getInsets(WindowInsetsCompat.Type.navigationBars())
            v.setPadding(navigationBars.left, statusBars.top, navigationBars.right, navigationBars.bottom)
            insets
        }
    } else {
        immersiveNavigationBar(callback)
    }
}
