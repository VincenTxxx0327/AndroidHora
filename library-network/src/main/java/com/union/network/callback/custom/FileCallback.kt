package com.union.network.callback.custom
...
import com.union.network.utils.FlyCacheUtils
import com.union.network.utils.NetworkUtil.DownloadStatus.STATUS_COMPLETE
import com.union.network.utils.NetworkUtil.DownloadStatus.STATUS_DEFAULT
import com.union.network.utils.NetworkUtil.DownloadStatus.STATUS_LOADING
import com.union.network.utils.createFile
...
abstract class FileCallback : NoCacheCustomCallback<File> {
    ...
    internal fun update(isBreakpointDownload: Boolean) {
        val isDownloading =
            FlyCacheUtils.getDownloadStatus(downloadFile.absolutePath) == STATUS_LOADING
        val isBreakpointDownloading = isBreakpointDownload && isDownloading  //是否正在断点下载中
        if (!isBreakpointDownloading) {
            FlyCacheUtils.setDownloadStatus(downloadFile.absolutePath, STATUS_DEFAULT)
    ...
    private inner class DownloadTask(private val responseBody: ResponseBody) :
        ...
        override fun onSuccess(result: File?) {
            ...
            launchMain {
                ...
                onSuccess(CacheResult.success(false, result))
                onComplete()
            }
        }
    }
    ...
    private fun saveFile(body: ResponseBody): File? {
        FlyCacheUtils.setDownloadStatus(downloadFile.absolutePath, STATUS_LOADING)
        ...
        try {
            ...
            launchMain {
                onError(CacheResult.error(false, e))
                onComplete()
            }
            return null
        } finally {
    ...
    private fun onProgress(progress: Progress, read: Int) {
        Progress.refreshProgress(progress, read.toLong()) {
            launchMain {
