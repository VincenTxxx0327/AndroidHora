...
class CacheAndRemoteDistinctStrategy : BaseStrategy() {
    ...
    override fun <T : Any> execute(rxCache: RxCache, cacheKey: String?, cacheTime: Long, source: Observable<T>, type: Type): Observable<CacheResult<T>> {
        ...
        return Observable.concat(cache, remote).filter { cacheResult ->
            cacheResult.data != null
        }.distinctUntilChanged { tCacheResult ->
            of(*tCacheResult.data.toString().toByteArray()).md5().hex()
        }
    }
}